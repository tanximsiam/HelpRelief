<template>
  <Modal v-if="!inlineMode" :show="showProp" title="NGO Report" @close="$emit('close')">
    <div class="max-w-3xl">
      <div v-if="successMessage" class="bg-green-100 text-green-800 px-4 py-2 rounded mb-4 text-center font-semibold">{{ successMessage }}</div>
...
const inlineMode = computed(() => !!props.inline)
const showProp = computed(() => !!props.show)
...
async function submitReport(payload: any) {
  ...
  try {
    const { data } = await api.post('/report/my-ngo', payload)
    successMessage.value = data?.message || 'Request submitted successfully!'
    // Do NOT emit('close') here; keep modal open
    emits('loaded', report.value)
  } catch (err: any) {
...
watch(() => props.inline, (v) => { if (v) loadReport() })

onMounted(() => { if (props.show || props.inline) loadReport() })
